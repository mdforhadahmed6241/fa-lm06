...
function faplm_register_api_settings() {
	...
	// --- RedEx (Replaced Cookie with Phone/Password) ---
	add_settings_field(
		'redex_phone',
		__( 'RedEx Phone', 'fa-pro-license-manager' ), // CHANGED
		'faplm_render_text_field', // Reuse existing text field renderer
		'fa-courier-settings',
		'faplm_direct_api_settings_section',
		array(
			'option_name' => 'faplm_courier_settings',
			'field_key'   => 'redex_phone', // CHANGED
			'description' => __( 'Phone number for the RedEx login bot (e.g., 017...).', 'fa-pro-license-manager' ), // CHANGED
		)
	);

	add_settings_field(
		'redex_password',
		__( 'RedEx Password', 'fa-pro-license-manager' ),
		'faplm_render_password_field', // Reuse existing password field renderer
		'fa-courier-settings',
		'faplm_direct_api_settings_section',
		array(
			'option_name' => 'faplm_courier_settings',
			'field_key'   => 'redex_password',
			'description' => __( 'Password for the RedEx login bot.', 'fa-pro-license-manager' ),
		)
	);
...
function faplm_render_password_field( $args ) {
	$value = faplm_get_setting( $args['field_key'], '' );
	?>
	<input type="password" name="faplm_courier_settings[<?php echo esc_attr( $args['field_key'] ); ?>]" value="<?php echo esc_attr( $value ); ?>" class="regular-text">
	<?php if ( ! empty( $args['description'] ) ) : ?>
		<p class="description"><?php echo esc_html( $args['description'] ); ?></p>
	<?php endif; ?>
	<?php
...
function faplm_sanitize_courier_settings( $input ) {
	...
	if ( isset( $input['hoorin_api_keys'] ) ) {
		// Allow newlines, but sanitize the content
	...
	// --- REMOVED 'redex_cookie' SANITIZATION ---
	/*
	if ( isset( $input['redex_cookie'] ) ) {
		// Cookies can contain special chars, but we still sanitize for safety
		$output['redex_cookie'] = sanitize_textarea_field( $input['redex_cookie'] );
	}
	*/

	// --- ADDED 'redex_phone' and 'redex_password' SANITIZATION ---
	if ( isset( $input['redex_phone'] ) ) {
		$output['redex_phone'] = sanitize_text_field( $input['redex_phone'] );
	}
	if ( isset( $input['redex_password'] ) ) {
		$output['redex_password'] = trim( $input['redex_password'] ); // Passwords should not be stripped
	}
	...
	if ( isset( $input['steadfast_password'] ) ) {
		// Passwords should not be stripped of special chars, so just trim
